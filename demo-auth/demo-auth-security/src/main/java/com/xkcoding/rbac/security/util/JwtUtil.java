...
public class JwtUtil {
  ...
  public String createJWT(
    Boolean rememberMe, Long id, String subject, List<String> roles, Collection<? extends GrantedAuthority> authorities
  ) {
    Date now = new Date();
    JwtBuilder builder = Jwts.builder()
      ...
      .issuedAt(now)
      .signWith(
        Keys.hmacShaKeyFor(jwtConfig.getKey().getBytes(StandardCharsets.UTF_8)),
        ALGORITHM
      )
      .claim("roles", roles)
  ...
  public Claims parseJWT(String jwt) {
    try {
      Claims claims = Jwts.parser().decryptWith(
        Keys.hmacShaKeyFor(jwtConfig.getKey().getBytes(StandardCharsets.UTF_8))
      )
